...
> 更新日期：2026-06-16  
> 当前实现进度：v0.5“协作可靠性”已完成第一版，核心写操作已接入角色权限、软删除、审计日志、锁船版本号和活动归档只读。
...
- 新增 `AuditLog`，数据导入、活动、锁船标签、锁船计划、作业卡、攻略档案等关键写操作会记录操作者、实体、前后状态摘要和时间。
- `RoutineRecord` / `StrategyPost` 已改为软删除路径，并预留 `isPinned` 置顶字段；列表、筛选和攻略插入作业卡入口默认排除已删除内容。
- `User.role` 已落地为实际权限：`member` 只能编辑自己内容，`planner` / `admin` 可管理公共活动、标签和全员锁船。
- `LockPlan.version` 已接入显式乐观锁，保存成功自增版本，版本不匹配返回冲突，减少多人覆盖修改风险。
- 活动归档改为 `status=archived`，归档活动仍可查看，但锁船、作业卡、攻略和标签写操作会进入只读保护。
- 锁船矩阵 UI 已按角色显示只读状态：普通成员查看他人锁船行时不可点选、移除或拖拽。
...
- `npm run build` 通过；lint 仍仅保留既有 `<img>` 优化 warning。

注意事项：

- `npx prisma migrate dev` 和 `prisma db push` 在当前本机环境触发 Prisma schema engine 空错误；schema 校验正常，因此本轮迁移以手写 SQLite migration 形式提交。
- v0.5 新增手写 migration：`20260616164000_collaboration_reliability`；部署环境需要执行迁移后再使用新的审计、软删除和版本字段。
